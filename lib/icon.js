...
function setIcon_callback() {
	if (chrome.runtime.lastError) {
		console.log("setIcon:",chrome.runtime.lastError.message);
	} else {
		// Tab exists
	}
}
...
const official_ip =  {'195.82.146.214':1, '81.17.30.51':1, };
...
function updateIcon(url, tabId) {
	...
	chrome.browserAction.setTitle({title:"RKN Alert",tabId:tabId},setIcon_callback);
	...
	if (info && tabId) {
		...
		if(ip) {
			icon_hint.reason_ip = info.reason[ip];
	...
	if (ip) {
		icon_hint.ip = ip;
		if (official_ip[ip]) icon_hint.not_fishing = true;
		//ip in whitelist?
	...
	if (localStorage.check_site_is_online == 1) {
		if (icon_hint.is_error || localStorage.check_site_only_if_error != 1) {
			icon_hint.check_is_online = check_if_is_online_cached(real_domain);
		}
	}
	...
	if ((bad_domains[domain] || domain.indexOf('rutracker') > -1 || bl_rkn_sites[real_domain] === 0
			|| domain.indexOf('nnmclub') > -1 || domain.indexOf('nnm-club') > -1
			)
			&& !bad_domain_whitelist[domain] && bl_rkn_sites[real_domain] !== 1 && !icon_hint.not_fishing) {
	...
	} else if (bad_domain_whitelist[domain] || bl_rkn_sites[real_domain] === 1 || icon_hint.not_fishing) {
		icon_hint.not_fishing = true;
		if (official_domain[domain]) icon_hint.official = true;
		if (!bad_domain_whitelist[domain]) icon_hint.is_personal_mirror = true;
	}
	
	//Check whois
	if (localStorage.show_age == 1 && icon_hint.ip !== domain) {
		//console.log('ip=',icon_hint.ip , domain);
		icon_hint.whois = getWhoisCached(real_domain);
	}
	...
	if (!rec) {
		...
		//Fishing
		if (icon_hint.is_fishing) {
			chrome.browserAction.setTitle({title:"Фишинговый сайт!",tabId:tabId},setIcon_callback);
			chrome.browserAction.setIcon({path: "images/circ_fishing_16.png"});
			return popup_update();
		}
		//No block
...
function onPopupOpen(popup) {
	...
	//accessibility
	if (icon_hint.check_is_online == 4) { //loading
		check_if_is_online(icon_hint.real_domain, popup.update_site_status);
	}
	//whois
	if (localStorage.show_age == 1 && icon_hint.ip !== icon_hint.real_domain && !getWhoisCached(icon_hint.real_domain)) {
		getWhois(icon_hint.real_domain, popup.update_whois);
	}
}
